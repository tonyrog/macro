...
%% macro & and ` operator
-export([eval_quote/2, expr_quote/1, match_quote/1]).
-export([make_constant/2]).
...
expr({call,_,{remote,_,{atom,_,'^macro'},{atom,_,Func}},As0}, Bs0, Lf, _Ef, RBs) ->
    local_func({macro,Func}, As0, Bs0, Lf, RBs);
...
expr({op,_,'`',A0}, Bs0, Lf, Ef, RBs) ->
    %% evaulate the backquote operator means traversing the structure and
    %% evalue only the &E expression found.
    A1 = eval_quote(A0, Bs0, Lf, Ef),
    ret_expr(A1, Bs0, RBs);
expr({op,_,'&',A0}, Bs0, _Lf, _Ef, RBs) ->
    %% partial evaluate A0
    ?dbg("expr & (~p)\n", [A0]),
    A1 = expr_peval(A0, Bs0),
    ?dbg("A1 = (~p)\n", [A1]),
    case get_constant(A1) of
	{true,V} ->
	    ret_expr(V, Bs0, RBs);
	false ->
	    ret_expr(A1, Bs0, RBs)
    end;
%% expr(A0, Bs0, Lf, Ef, RBs);
...
local_func(Func, As0, Bs0, {M,F}, RBs) ->
    {As1,Bs1} = lexpr_list(Func, As0, Bs0, {M,F}),
...
lexpr_list({macro,_F}, As, Bs, _Lf) ->
    {As, Bs};
lexpr_list(_Func, As, Bs, Lf) ->
    expr_list(As, Bs, Lf).
...
list_to_conses([], Line) -> 
    {nil,Line};
list_to_conses([H|T], Line) ->
    H1 = if is_integer(H) -> {integer,Line,H};
	    is_float(H)   -> {float,Line,H};
	    is_atom(H)    -> {atom,Line,H};
	    is_list(H)    -> list_to_conses(H,Line)
			     %% is_binary(H) ->
	 end,
    {cons, Line, H1, list_to_conses(T, Line)}.
...
match1({op,_,'&',{var,_,'_'}}, _Term, Bs, _BBs) ->
    {match,Bs};	% everything, no new bindings
match1({op,_,'&',{var,_,Name}}, Term, Bs, _BBs) ->
    ?dbg("eval: &match1 Name=~p\n", [Term]),
    case binding(Name, Bs) of
	{value,Term} ->
	    {match,Bs};
	{value,_} ->
...
match1({op,_,'&',A}, Term, Bs, BBs) ->
    A1 = expr_peval(A, Bs),
    match1(A1, Term, Bs, BBs);
match1({op,_,'`',A}, Term, Bs, BBs) ->
    match1(match_quote(A), Term, Bs, BBs);
...
%% Scan structure for the '&' operator and evaluate
eval_quote(E, Bs) ->
    eval_quote(E, Bs, none, none).

eval_quote([E|Es], Bs, Lf, Ef) ->
    [eval_quote(E,Bs,Lf,Ef) | eval_quote(Es,Bs,Lf,Ef)];
eval_quote([], _Bs, _Lf, _Ef) ->
    [];
eval_quote(Expr, Bs, Lf, Ef) ->
    case Expr of
	{op,_,'&', E} ->
	    expr_peval(E, Bs);
	{Tag,Line,E1} ->
	    {Tag,Line,eval_quote(E1,Bs,Lf,Ef)};
	{Tag,Line,E1,E2} ->
	    {Tag,Line,
	     eval_quote(E1,Bs,Lf,Ef),
	     eval_quote(E2,Bs,Lf,Ef)};
	{Tag,Line,E1,E2,E3} ->
	    {Tag,Line,
	     eval_quote(E1,Bs,Lf,Ef),
	     eval_quote(E2,Bs,Lf,Ef),
	     eval_quote(E3,Bs,Lf,Ef)};
	{Tag,Line,E1,E2,E3,E4} ->
	    {Tag,Line,
	     eval_quote(E1,Bs,Lf,Ef),
	     eval_quote(E2,Bs,Lf,Ef),
	     eval_quote(E3,Bs,Lf,Ef),
	     eval_quote(E4,Bs,Lf,Ef)};
	Expr1 ->
	    Expr1
    end.

%% Abstractify expression 
expr_quote([E|Es]) ->
    [expr_quote(E) | expr_quote(Es)];
expr_quote([]) ->
    [];
expr_quote(Expr) ->
    case Expr of
	{op,_,'&', E} ->
	    expr_peval(E, new_bindings());
	{Tag,Line,E1} ->
	    {tuple,0,[{atom,0,Tag},{integer,0,Line},
		      expr_quote(E1)]};
	{Tag,Line,E1,E2} ->
	    {tuple,0,[{atom,0,Tag},{integer,0,Line},
		      expr_quote(E1),expr_quote(E2)]};
	{Tag,Line,E1,E2,E3} ->
	    {tuple,0,[{atom,0,Tag},{integer,0,Line},
		      expr_quote(E1),expr_quote(E2),expr_quote(E3)]};
	{Tag,Line,E1,E2,E3,E4} ->
	    {tuple,0,[{atom,0,Tag},{integer,0,Line},
		      expr_quote(E1),expr_quote(E2),
		      expr_quote(E3),expr_quote(E4)]};
	T when is_atom(T) -> {atom,0,T};
	T when is_integer(T) -> {integer,0,T};
	T when is_float(T) -> {integer,0,T}
    end.


%% Abstractify expression for match
match_quote([E|Es]) ->
    [match_quote(E) | match_quote(Es)];
match_quote([]) ->
    [];
match_quote(Expr) ->
    case Expr of
	{op,_,'&', E} ->
	    expr_peval(E, new_bindings());
	{Tag,_Line,E1} ->
	    {tuple,0,[{atom,0,Tag},{var,0,'_'},
		      match_quote(E1)]};
	{Tag,_Line,E1,E2} ->
	    {tuple,0,[{atom,0,Tag},{var,0,'_'},
		      match_quote(E1),match_quote(E2)]};
	{Tag,_Line,E1,E2,E3} ->
	    {tuple,0,[{atom,0,Tag},{var,0,'_'},
		      match_quote(E1),match_quote(E2),match_quote(E3)]};
	{Tag,_Line,E1,E2,E3,E4} ->
	    {tuple,0,[{atom,0,Tag},{var,0,'_'},
		      match_quote(E1),match_quote(E2),
		      match_quote(E3),match_quote(E4)]};
	T when is_atom(T)    -> {atom,0,T};
	T when is_integer(T) -> {integer,0,T};
	T when is_float(T)   -> {integer,0,T}
    end.
%%
%% Recursive partial eval of constant expression and substitutions
%% handle things like:
%%     1+2 => 3
%%     true andalso X => X
%%     false andalso Y => false
%%     ...
%%     (fun(X) -> X+1 end)(3) => (3)+1 => 4
%%     (fun(X) -> X+1 end)(Z) => (Z)+1
%%
expr_peval(E,Bs) ->
    case E of
	...
	{op,Ln,'`',A} ->
	    A1 = expr_peval(A,Bs),
	    try make_constant(Ln,A1) of
		Const -> Const
	    catch
		error:_ -> A1
	    end;
	{op,_Ln,'&',A} -> expr_peval(A,Bs);
	{op,Ln,Op,A}  -> expr_peval_op(Op, Ln, expr_peval(A,Bs));
	{op,Ln,'andalso',A,B} ->
	    case expr_peval(A,Bs) of
		{atom,_,false} -> {atom,Ln,false};
		{atom,_,true} -> expr_peval(B,Bs);
		Ax -> {op,Ln,'andalso',Ax,expr_peval(B,Bs)}
	    end;
	{op,Ln,'orelse',A,B} ->
	    case expr_peval(A,Bs) of
		{atom,_,true} -> {atom,Ln,true};
		{atom,_,false} -> expr_peval(B,Bs);
		Ax -> {op,Ln,'orelse',Ax,expr_peval(B,Bs)}
	    end;
	{op,Ln,Op,A,B} -> expr_peval_op(Op, Ln, expr_peval(A,Bs), expr_peval(B,Bs));
	{tuple,Ln,Es}  -> {tuple,Ln,expr_peval(Es,Bs)};
	{cons,Ln,H,T}  ->
	    case {expr_peval(H,Bs),expr_peval(T,Bs)} of
		{{char,_,C},{string,_,S}} ->
		    {string,Ln,[C|S]};
		{{integer,_,C},{string,_,S}} when C >= 0, C=< 255 ->
		    {string,Ln,[C|S]};
		{{char,_,C},{nil,_}} ->
		    {string,Ln,[C]};
		{{integer,_,C},{nil,_}} when C >= 0, C=< 255 ->
		    {string,Ln,[C]};
		{H1,T1} -> {cons,Ln,H1,T1}
	    end;
	%% fixme: {lc,_,E,Qs}
	%% fixme: {bc,_,E,Qs}
	{block,Ln,Es} ->
	    case expr_peval(Es,Bs) of
		[E] -> E;
		Es1 -> {block,Ln,Es1}
	    end;
	%% {'if',Ln,Cs} ->
	    %% remove all false clauses, if first none false clause
	    %% is true clause then remove the if and return that body
	%% expr_peval_if_clauses(Cs);
	%% {'case',Ln,E,Cs} ->
	%% E1 = expr_peval(E),
	%%  expr_peval_case_clauses(E1,Cs);
	%% fixme: try/receive/fun
	%%
	%% handle element(<int>, {E1,E2,E3...En})
	{call,L1,{atom,L2,element},[Index,Tuple]} ->
	    I = expr_peval(Index,Bs),
	    T = expr_peval(Tuple,Bs),
	    case {I,T} of
		{{integer,_,I1},{tuple,_,Es}} when I1>0,I1=<length(Es)->
		    case erl_syntax:is_literal(T) of
			true ->
			    lists:nth(I1,Es);
			false ->
			    {call,L1,{atom,L2,element},[I,T]}
		    end;
		_ ->
		    {call,L1,{atom,L2,element},[I,T]}
	    end;
...
expr_peval_op(Op, Ln, L, R) ->
    case {get_constant(L),get_constant(R)} of
	{{true,A},{true,B}} ->
	    try make_constant(Ln,erlang:Op(A, B)) of
		Const -> Const
	    catch
		error:_ -> {op,Ln,Op,L,R}
	    end;
	_ -> {op,Ln,Op,L,R}
    end.

expr_peval_op(Op, Ln, M) ->
    case get_constant(M) of
	{true,A} ->
	    try make_constant(Ln,erlang:Op(A)) of
		Const -> Const
	    catch
		error:_ -> {op,Ln,M}
	    end;
...
make_constant(Ln,C) when is_integer(C) -> {integer,Ln,C};
make_constant(Ln,C) when is_float(C) ->   {float,Ln,C};
make_constant(Ln,C) when is_atom(C) ->    {atom,Ln,C};
make_constant(Ln,C) when is_list(C) ->
    case is_string(C) of
	true -> {string,Ln,C};
	false -> list_to_conses(C, Ln)
    end;
make_constant(Ln,C) when is_binary(C) ->
    {bin,Ln,
     [{bin_element,Ln,{integer,Ln,X},default,default} ||
	 <<X:8>> <= C]}.

is_string([H|T]) when H >= 0, H =< 255 ->
    is_string(T);
is_string([_|_]) -> false;
is_string([]) -> true.

%% Check constant args (add binary)
get_constant({integer,_,X}) -> {true,X};
get_constant({char,_,X})    -> {true,X};
get_constant({float,_,X})   -> {true,X};
get_constant({nil,_})       -> {true,[]};
get_constant({string,_,X})  -> {true,X};
get_constant({atom,_,X})    -> {true,X};
get_constant(_)             -> false.
